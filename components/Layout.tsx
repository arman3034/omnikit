...
import { Menu, X, Box, Home, Info, Mail, ChevronDown, Type, Sparkles, QrCode, FileText, Lock, Moon, Sun, Palette, GraduationCap } from 'lucide-react';
...
const NAV_ITEMS: NavConfig[] = [
  ...
  { path: '/tools', label: 'Tools', icon: Box, shortcut: 'T' },
  { path: '/about', label: 'About Us', icon: Info, shortcut: 'A' },
  { path: '/contact', label: 'Contact Us', icon: Mail, shortcut: 'C' },
];

const NAV_SUB_TOOLS = [
  { id: 'cgpa-companion', label: 'CGPA Companion', icon: GraduationCap, path: '/tools?tool=cgpa-companion' },
...
const Layout: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.altKey && !e.ctrlKey && !e.shiftKey && !e.metaKey) {
        ...
        const navItem = NAV_ITEMS.find(item => item.shortcut?.toLowerCase() === key);
  ...
  // Update time every second with greeting
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      ...
      let greeting = 'Good Evening! Hope you had a great day';
      if (hour >= 5 && hour < 12) {
        greeting = 'Good Morning! Ready to get things done?';
      ...
      setCurrentTime(`${greeting} - ${timeString}`);
    };
      ...
      <header className="sticky top-0 z-50 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 shadow-sm supports-[backdrop-filter]:bg-white/80 dark:supports-[backdrop-filter]:bg-slate-900/80 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16 md:h-20 transition-all">
            ...
            {currentTime && (
              <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 rounded-xl border border-indigo-100 dark:border-indigo-900/50 shadow-sm">
                <span className="text-sm font-semibold text-indigo-700 dark:text-indigo-300" aria-live="polite" aria-atomic="true">
                  {currentTime}
                </span>
              </div>
            )}
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <NavLink
                  ...
                  `}
                >
                  <item.icon size={22} className={`flex-shrink-0 transition-colors ${isActive ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} />
                  <span className="flex-1">
                    {item.label}{item.shortcut && ` (Alt+${item.shortcut})`}
                  </span>
                </NavLink>
              );
            })}
