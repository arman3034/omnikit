...
export const TextConverter: React.FC = () => {
  ...
  const transform = useCallback((type: 'upper' | 'lower' | 'title' | 'sentence') => {
    setText((prevText) => {
      let result = prevText;
      switch (type) {
        case 'upper': result = prevText.toUpperCase(); break;
        case 'lower': result = prevText.toLowerCase(); break;
        case 'title': 
          result = prevText.replace(/\w\S*/g, (txt) => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase());
          break;
        case 'sentence':
          result = prevText.toLowerCase().replace(/(^\s*\w|[.!?]\s*\w)/g, (c) => c.toUpperCase());
          break;
      }
      return result;
    });
