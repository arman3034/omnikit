...
import { Plus, Trash2, RefreshCw, Download, Save, User, GraduationCap, ChevronDown, ChevronUp, Copy, Calculator } from 'lucide-react';
...
// Using a new storage key to reset potentially corrupted data from previous versions
const STORAGE_KEY = 'omnikit_semester_gpa_data_v1';

// Robust ID Generator to prevent key collisions
...
const CgpaCalculator: React.FC = () => {
  ...
  const [profile, setProfile] = useState<StudentProfile>(() => {
    // Only used for initial render if no local storage
    ...
    if (saved) {
      try {
        ...
        if (parsed.semesters && Array.isArray(parsed.semesters)) {
          // Ensure every semester and course has a valid ID
          const sanitizedProfile: StudentProfile = {
             ...parsed,
             semesters: parsed.semesters.map((s: Semester) => ({
                ...s,
                id: s.id || generateId(),
                courses: s.courses.map((c: CourseInput) => ({ ...c, id: c.id || generateId() }))
             }))
          };
          setProfile(sanitizedProfile);
          
          if (sanitizedProfile.semesters.length > 0) {
             setExpandedSemesters(prev => {
                if (Object.keys(prev).length === 0) {
                   return { [sanitizedProfile.semesters[0].id]: true };
                }
                return prev;
             });
          }
  ...
  const removeSemester = (semId: string) => {
    ...
    if (!window.confirm("Delete this semester and all its courses? This cannot be undone.")) return;

    setProfile(prev => {
      const nextSemesters = prev.semesters.filter(s => s.id !== semId);
      const nextProfile = { ...prev, semesters: nextSemesters };
      try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(nextProfile));
      } catch (e) {
        console.warn("Failed to persist after delete", e);
      }
      return nextProfile;
    });
    ...
    setExpandedSemesters(prev => {
      const next = { ...prev };
      if (next[semId]) delete next[semId];
      return next;
    });
  ...
  const handleReset = () => {
    if (window.confirm("Are you sure you want to clear all semesters and student data? This cannot be undone.")) {
      localStorage.removeItem(STORAGE_KEY);
      initializeDefaults();
      setOverallResult(null);
      showToast("Calculator data reset.", "info");
  ...
  const cloneSemester = (semId: string) => {
    ...
    setProfile(prev => ({ ...prev, semesters: [...prev.semesters, newSem] }));
    setExpandedSemesters(prev => ({ ...prev, [newSem.id]: true }));
    showToast('Semester structure cloned', 'success');
  };
    ...
    <div className="space-y-8 cgpa-tool">
      ...
      <div className="flex flex-col xl:flex-row gap-6 items-start">
            ...
            {profile.semesters.map((semester, semIdx) => {
               ...
               return (
                  <div key={semester.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-300">
                     ...
                     <div 
                        className={`
                            p-4 flex items-center justify-between select-none transition-colors
                        ...
                        `}
                        onClick={(e) => {
                            if (e.target === e.currentTarget) toggleSemester(semester.id);
                        }}
                     >
        ...
        <div className="w-full xl:w-80 flex-shrink-0 space-y-6 sticky top-24">
           ...
           <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-xl p-4 text-xs text-indigo-900 dark:text-indigo-200 flex items-start gap-2">
               <Save size={14} className="mt-0.5 shrink-0" aria-hidden="true" />
               <p className="opacity-80">Auto-saving enabled. Your progress is saved locally.</p>
           </div>
        </div>
      </div>
    </div>
